...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, DropOut=False):
        ...
        self.dropout = nn.Dropout(1-0.5, inplace=False)
        self.relu = nn.ReLU(inplace=False)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0],layer_index=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False,layer_index=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, DropOut=drop_out))
                           
        self.inplanes = planes * block.expansion
        for blk in range(1, blocks):
            if layer_index == 4 and blk == 1:
                drop_out = True
            else:
                drop_out = False    
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, DropOut=drop_out))                  
